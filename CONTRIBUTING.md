...
- **One reconciler, many front-ends.** The dashboard, the `helmsman apply` CLI, and SSH all produce
...
[docs/security.md](./docs/security.md) §15) **before merge**, and needs an explicit security review:
...
- ACME / cert handling
- the edge config renderer and the raw-config editor
...
- Custom lint rules (must stay green): no `exec.Command` with request/DB/app-derived args outside the
  §5.6 validator; no `sh -c`; no un-confined path from external input; no `text/template` /
...
maintainers (see `SECURITY.md` for the disclosure address) so a fix can ship before disclosure.
