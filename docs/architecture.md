...
- **Docker access is hybrid:** **reads** go through the read-only socket-proxy via the Docker Go SDK; **writes** shell out to `docker compose` with **static argv only** — never a shell, never string interpolation, always `--` terminators.
