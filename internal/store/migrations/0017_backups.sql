-- Backups catalog: each row is one encrypted snapshot of Helmsman's own state (the
-- SQLite DB, taken consistently via VACUUM INTO, then AES-256-GCM-encrypted with the
-- master key). This is the "recover Helmsman onto a fresh box" backup — it captures
-- every app's config, secrets (already encrypted), definitions, edge routes, etc.
-- The archive file lives under <data_dir>/backups/<id>.hmbk; only the catalog row and
...
CREATE TABLE backups (
    ...
    kind       TEXT NOT NULL DEFAULT 'helmsman-state',
