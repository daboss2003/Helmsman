...
// Inspect must accept a real Mooring DB and REJECT a blank or foreign SQLite file —
// the guard that stops `restore` from installing a bogus archive over the live DB.
func TestInspect(t *testing.T) {
	dir := t.TempDir()

	// A real Mooring DB (migrated by Open).
	...
	// A 0-byte file → not a Mooring DB.
	...
	// A valid SQLite file with only foreign tables → not a Mooring DB.
