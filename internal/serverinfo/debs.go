...
// debRe matches ONLY a Helmsman release .deb. The cleanup feature will never look
...
// busy directory.
var debRe = regexp.MustCompile(`^helmsman_([0-9]+\.[0-9]+\.[0-9]+)_linux_(amd64|arm64)\.deb$`)

// ErrNotADeb is returned when a delete target isn't a recognized Helmsman .deb.
var ErrNotADeb = errors.New("serverinfo: not a helmsman .deb")
...
// Deb is one Helmsman release package found in the configured cache dir.
type Deb struct {
	Name    string
	Version string
	Arch    string
...
// ListDebs returns the Helmsman .deb files directly inside dir (non-recursive),
...
// DeleteDeb removes one Helmsman .deb from dir. It is intentionally narrow: name
